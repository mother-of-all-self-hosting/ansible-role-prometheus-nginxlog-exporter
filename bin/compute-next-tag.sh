...
#
# Tags look like `v<prometheus-nginxlog-exporter version>-<release>`:
#
# - if defaults/main.yml points at an exporter version that has never been
#   released, the release counter restarts at 0 (`v1.11.0-0`)
# - otherwise the counter is incremented (`v1.11.0-1`), but only if something
...
# `prometheus_nginxlog_exporter_version` is the leaf literal that Renovate
# bumps. The container image tag is derived from it in Jinja (it appends the
# architecture suffix the upstream registry publishes under), so it is not a
# value this script could read.
version="$(sed -nE 's|^prometheus_nginxlog_exporter_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the prometheus-nginxlog-exporter version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v1.10.0`),
